...
from literal_search import literal_profession_for_job
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build Masari multi-source public search index with source-aware hard gates")
    ...
    aggregate_classifiability = evaluate_classifiable_coverage(indexed, minimum_coverage_pct=args.min_coverage)
    emploi_public_jobs = [j for j in indexed if _source(j) != "anapec"]
    emploi_public_gate = evaluate_classifiable_coverage(emploi_public_jobs, minimum_coverage_pct=args.min_coverage)

    anapec_jobs = [j for j in indexed if _source(j) == "anapec"]
    anapec_missing_literal = [
        {"uuid": j.get("uuid"), "source_offer_id": j.get("source_offer_id"), "title": j.get("job_name") or j.get("title")}
        for j in anapec_jobs
        if not isinstance(j.get("literal_profession"), dict)
    ]
    anapec_literal_jobs = len(anapec_jobs) - len(anapec_missing_literal)
    anapec_literal_coverage = round(anapec_literal_jobs / len(anapec_jobs) * 100, 2) if anapec_jobs else 100.0
    anapec_gate = not anapec_missing_literal and anapec_literal_coverage == 100.0
    gate = bool(emploi_public_gate["gate"] and anapec_gate)

    source_counts = dict(sorted(Counter(_source(job) for job in indexed).items()))
    canonical_classified_by_source = dict(sorted(Counter(_source(job) for job in indexed if job.get("profession_matches")).items()))
    ...
    taxonomy_audit = {
        ...
        "source_gate_policy": {
            ...
            "anapec": {
                ...
                "jobs": len(anapec_jobs),
                "literal_searchable_jobs": anapec_literal_jobs,
                "literal_searchability_coverage_pct": anapec_literal_coverage,
                "required_pct": 100.0,
                "canonical_taxonomy_classified_jobs": canonical_classified_by_source.get("anapec", 0),
                "literal_fallback_jobs": len(literal_fallback),
                "missing_literal_jobs": len(anapec_missing_literal),
                "gate": "PASS" if anapec_gate else "FAIL",
            },
        },
        "coverage_gate_policy": "SOURCE_AWARE_HARD_GATES_NO_FABRICATED_TAXONOMY",
        "searchable_match_policy": "CANONICAL_EXACT_STRONG_OR_ANAPEC_VERIFIED_LITERAL_TITLE",
    ...
    for name, data in {
        ...
        "anapec_missing_literal_jobs.json": anapec_missing_literal,
